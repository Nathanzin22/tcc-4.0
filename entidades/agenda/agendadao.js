...
class AgendaDao {
    async salvar(agenda) {
        const query = 'INSERT INTO agendas (data, horario, disponibilidade, profissao, user_id, obs) VALUES ($1, $2, $3, $4, $5, $6)';
        const values = [agenda.data, agenda.horario, agenda.disponibilidade, agenda.profissao, agenda.user_id, agenda.obs];
    ...
    async editar(agenda) {
        const query = 'UPDATE agendas SET disponibilidade = $1, data = $2, horario = $3, obs = $4 WHERE id = $5';
        const values = [agenda.disponibilidade, agenda.data, agenda.horario, agenda.obs, agenda.id];
